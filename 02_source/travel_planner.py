...
def main():
    ...
    # 만약 argparse가 인자 에러를 만나면 기본 사용법을 출력함.
    # 커스텀 검증은 validate_date로 진행.
    ...
    # API 키 검증 (LLM과 지도 API 둘 중 최소 1개는 키를 가지고 있어야 함)
    if not llm_client.has_valid_key() or not map_client.has_valid_key():
        print_api_guide()
        sys.exit(1)
        
    print(f"\n>>> {date_str} 날짜의 여행 계획 생성을 시작합니다.")
