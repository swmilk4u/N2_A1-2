...
2. **템플릿 제공 및 가이드 분리**: 협업 및 배포 환경에서의 정보 노출 방지를 위해 실제 키 값을 지운 [.env.template](file:///g:/내 드라이브/20260415 코디세이 AI 네이티브 과정/02_mission/N2_A1-2_Trip service/.env.template)만을 리포지토리에 제공하여 안전성을 확보했습니다.
3. **Git 추적 배제 (.gitignore)**: 실제 작동에 사용되는 `.env` 파일 및 실행 날짜 기준으로 자동 생성되는 로컬 데이터 폴더(`results/`)가 Git 버전 관리에 절대 수집되지 않도록 최상위 [.gitignore](file:///g:/내 드라이브/20260415 코디세이 AI 네이티브 과정/02_mission/N2_A1-2_Trip service/.gitignore)에 차단 규칙을 등록하여 우발적인 유출 사고를 방지했습니다.
