...
내가 직접 개발한 이 프로그램은 사용자가 입력한 특정 날짜를 기반으로 **Google Gemini API**와 **Naver Local Search API**를 유기적으로 연동하여 맞춤형 여행 코스 및 맛집 리스트를 생성하는 CLI 프로그램입니다.

특히 내가 구현한 소스 코드와 핵심 연동 원리를 타인에게 명확하고 쉽게 설명할 수 있도록 주요 설명 요약과 코드 조각을 수록했습니다.
...
> **💡 [핵심 연동 개념 이해]**
> - 내가 설계한 **파이프라인(Pipeline)**이란 첫 번째 모듈(Gemini)이 가공하고 추천한 도시 이름을 전달받아, 두 번째 모듈(네이버 지도 검색)이 바로 입력으로 연결하여 최종 여행 리포트라는 하나의 결과물을 완성해내는 유기적인 연결 통로입니다.
...
> **💡 [나의 코드 설명 포인트]**
> "일반적인 CLI 프로그램은 인자 없이 그냥 더블클릭하면 1초 만에 켜졌다가 꺼져 버립니다. 내가 구현한 프로그램은 **argparse** 옵션이 비어있으면 스스로 **대화형 입력 모드**로 전환하여 창 닫힘을 방지하고, 사용자가 입력창에 옵션 기호를 섞어 쓰는 실수를 하더라도 **정규식(Regular Expression)**을 가동해 날짜만 정밀하게 정제하여 정상적으로 실행되도록 예외 처리를 완료했습니다."
...
> **💡 [나의 코드 설명 포인트]**
> "LLM(인공지능)의 답변은 때로 비구조적인 일반 텍스트로 오기 때문에 파이프라인의 다음 단계 입력으로 쓰기 어렵습니다. 따라서 나는 Gemini API 호출 시 JSON 출력을 명시적으로 요구하고, 파이팅 에러 발생 시 예외 처리를 거쳐 프롬프트를 보강해 **최대 1회 재요청**을 보내도록 코드를 설계하여 비즈니스 로직의 결합 안정성을 극대화했습니다."
...
> **💡 [나의 코드 설명 포인트]**
> "외부 API 서비스는 네트워크 사정이나 허용 쿼터 초과 등으로 언제든 작동이 정지될 위험이 있습니다. 나는 이를 극복하고자 **오류 격리(Error Isolation)** 기법을 코딩에 도입하여, 네이버 지도 API에 통신 장애가 나더라도 예외 처리를 거쳐 오류 리스트에 상세 기록만 기록해둔 뒤, 최종 여행 계획 리포트 생성 단계는 정상적으로 진행되도록 완성도를 높였습니다."
...
> **💡 [나의 코드 설명 포인트]**
> "똑같은 날짜로 다시 실행할 때마다 고비용의 인공지능 API 및 네트워크 검색을 반복 호출하는 것은 비효율적입니다. 내가 추가 구현한 **결과 캐싱(Caching)** 기능은 이미 수집된 결과가 있으면 API 연동 단계를 통째로 생략하여 연산 속도를 1초 미만으로 단축하고, API 요금을 대폭 절감합니다."

---

## 3. 🔒 API 키 관리 및 보안 준수 사항

나는 과제의 보안 및 비공개 수행 규칙을 엄격히 준수하여 API 키 유출을 원천 차단하고 안전하게 관리하도록 설계 및 구현했습니다.
