...
def main():
    ...
    # 실행할 때 날짜를 지정하지 않았다면, 화면에서 날짜를 직접 입력받도록 작동 (더블클릭 실행 시 창이 바로 꺼지는 현상 방지)
    if not date_str:
...
- **오류 격리 및 대체 결과 제공 (폴백)**: API 키 인증 오류(401 : 비인증 /403 : 권한 없음) 또는 검색 결과가 0건일지라도 프로그램이 비정상 종료(Crash)되지 않도록 `try-except`로 오류를 감싸고, 에러 이력을 남긴 채 **"데이터 없음"** 상태로 다음 단계(마크다운 리포트 생성)를 차분히 이어 나갑니다.
...
- **인증 에러 (AUTH_ERROR, 401 : 비인증 / 403 : 권한 없음)**:
  - *원인*: 유효하지 않은 API 키 사용 혹은 변수명 오타(401 : 비인증), 또는 네이버 개발자 센터 내 API 권한 신청 누락 및 무료 사용량 초과(403 : 권한 없음).
  - *대응*: 오류 목록(`errors`)에 인증 오류 상태를 기록하고, 맛집 추천 정보는 "데이터 없음" 처리 후 프로그램 중단 없이 리포트 생성을 계속 진행합니다.
